...
////////////////////////////////////////////// **********************************************************************
uint8_t frame_buffer[2 * 320 * 240];
uint8_t frame_buffer1[320 * 240];
...
void captureImg(uint16_t wg, uint16_t hg){
    ...
    // Wait for VSYNC to go low, indicating start of frame
    while (((*vsync) & VSYNC_PIN) != PIN_LOW);  // Wait for high

    // Wait for VSYNC to go high, indicating end of frame start signal
    while (((*vsync) & VSYNC_PIN) == PIN_LOW);  // Wait for low

    // Iterate over each line
    for (y = 0; y < hg; y++) {
        // Iterate over each pixel in the line
        for (x = 0; x < wg; x++) {
            // Wait for pixel clock to signal data is ready
            while (((*pixclk) & PXCLK_PIN) == PIN_LOW);  // Wait for pixel clock low
            uint8_t pixel_data = (*data_pins) & 0xFF;    // Read data pins
            HAL_UART_Transmit(&huart1, &pixel_data, 1, HAL_MAX_DELAY);  // Send data

            // Wait for the next pixel clock
            while (((*pixclk) & PXCLK_PIN) != PIN_LOW);  // Wait for pixel clock high
            while (((*pixclk) & PXCLK_PIN) == PIN_LOW);  // Wait for pixel clock low again
        }
    }
    HAL_Delay(1);  // Small delay after frame capture
}
...
//      uint8_t pixel_data = (*data_pins);
//      HAL_UART_Transmit(&huart1, &pixel_data, 1, HAL_MAX_DELAY);
...
//void captureImg(uint16_t wg, uint16_t hg) {
//    uint16_t y, x;
//    uint32_t buffer_index = 0;  // Index for the buffer
//
//    __IO uint32_t *vsync = &(VSYNC_PORT->IDR);
//    __IO uint32_t *pixclk = &(PXCLK_PORT->IDR);
//    __IO uint32_t *data_pins = &(DATA_PORT->IDR);
...
//    for (y = 0; y < hg; y++) {
//        for (x = 0; x < wg; x++) {
//            while (((*pixclk) & (PXCLK_PIN)) == PIN_LOW);  // Wait for the pixel clock low
//            frame_buffer1[buffer_index++] = (*data_pins);   // Store pixel data in buffer
//            while (((*pixclk) & (PXCLK_PIN)) != PIN_LOW);  // Wait for the pixel clock high
//        }
//    }
//    HAL_Delay(100);  // Delay for any further processing
//}
...
// Capture and process a single block of the image
void capture_block(int start_row, int start_col, int block_width, int block_height) {
    __IO uint32_t *vsync = &(VSYNC_PORT->IDR);
    __IO uint32_t *hsync = &(HSYNC_PORT->IDR);
    __IO uint32_t *pixclk = &(PXCLK_PORT->IDR);
    __IO uint32_t *data_pins = &(DATA_PORT->IDR);
    ...
    while ((*vsync & VSYNC_PIN) == PIN_LOW) {
        int row_count = 0;
        while (row_count < start_row) {  // Skip rows until we reach the start of the block
            while ((*hsync & HSYNC_PIN) != PIN_LOW);  // Wait for HSYNC to start a new row
            row_count++;
        }

        for (int y = 0; y < block_height; y++) {
            int col_count = 0;
            while ((*hsync & HSYNC_PIN) != PIN_LOW);  // Start a new row
            while (col_count < start_col) {  // Skip pixels until we reach the start of the block
                while ((*pixclk & PXCLK_PIN) == PIN_LOW);
                while ((*pixclk & PXCLK_PIN) != PIN_LOW);
                col_count++;
            }
            ...
            for (int x = 0; x < block_width; x++) {
                while ((*pixclk & PXCLK_PIN) == PIN_LOW);  // Wait for pixel clock low
                uint8_t pixel_data = (*data_pins);// & 0xFFFF;  // Assume data_pins has 16 bits of valid data
                block_buffer[y * block_width + x] = pixel_data;  // Store pixel data
                HAL_UART_Transmit(&huart1, pixel_data, 1, HAL_MAX_DELAY);
//                printf(pixel_data);
                while ((*pixclk & PXCLK_PIN) != PIN_LOW);  // Wait for pixel clock high
            }
...
//        process_image_block(block_buffer, block_width, block_height);
        break;  // Exit after processing the required block
    }

    __enable_irq();
}

void transmit_block(uint16_t* block_buffer, int buffer_size, uint8_t format) {
    uint8_t pixel[3]; // To store converted pixel values
    uint8_t buf[960]; // UART transmission buffer
    int count = 0;    // Index for transmission buffer

    // Loop through each pixel in the block buffer
    for (int i = 0; i < buffer_size; i++) {
        // Extract RGB565 data from block buffer
//        uint16_t pixel_data = block_buffer[i];
//        pixel[0] = (pixel_data >> 8) & 0xF8; // Extract red component
//        pixel[1] = ((pixel_data >> 3) & 0xFC) | ((pixel_data >> 5) & 0x07); // Extract green component
//        pixel[2] = (pixel_data & 0x1F) << 3; // Extract blue component
//
//        // Scale to 8-bit per channel
//        pixel[0] = (pixel[0] * 255) / 248; // 5-bit to 8-bit
//        pixel[1] = (pixel[1] * 255) / 252; // 6-bit to 8-bit
//        pixel[2] = (pixel[2] * 255) / 248; // 5-bit to 8-bit
        ...
        // Handling different formats for transmission
        if (format) { // Binary format (P6)
            buf[count++] = r8;
            buf[count++] = g8;
            buf[count++] = b8;
//            buf[count++] = pixel[0];
...
//                       buf[count++] = pixel[2];
            ...
            // Transmit the buffer when full or at the end
            if (count == 960 || i == buffer_size - 1) {
                HAL_UART_Transmit(&huart1, buf, count, HAL_MAX_DELAY);
                HAL_Delay(25); // Delay to prevent UART overflow
                count = 0; // Reset the buffer index
            }
        } else { // ASCII format (P3)
            char temp[30];
            int len = snprintf(temp, 30, "%d %d %d ", pixel[0], pixel[1], pixel[2]);
            if (count + len < 960) {
