...
static const regval_list vga_ov7670[]  = {
...
static const regval_list qvga_ov7670[]  = {
	{REG_COM14, 0x19},
	{0x72, 0x11},
	{0x73, 0xf1},
	{REG_HSTART,0x16},
...
static const regval_list qqvga_ov7670[]  = {
...
static const regval_list yuv422_ov7670[]  = {
...
static const regval_list rgb565_ov7670[]  = {
...
static const regval_list bayerRGB_ov7670[]  = {
	{REG_COM7, COM7_BAYER},
...
static const regval_list ov7670_default_regs[]  = {//from the linux driver
	{REG_COM7, COM7_RESET},
	{REG_TSLB,  0x04},	/* OV */
	{REG_COM7, 0},	/* VGA */
	/*
...
uint8_t wrReg(uint8_t reg_address, uint8_t data)
{
  ...
  while ((check != HAL_OK) && (counter != MAX_I2C_RETRIES_))
  {
    check = HAL_I2C_Mem_Write(&hi2c1, OV7670_WRITE_ADDR_, reg_address, I2C_MEMADD_SIZE_8BIT, &data, 1, MAX_I2C_TIMEOUT_);
    counter++;
  }
...
uint8_t rdReg(uint8_t reg)
{
    ...
    status = HAL_I2C_Master_Transmit(&hi2c1, OV7670_WRITE_ADDR_ << 1, &reg, 1, MAX_I2C_TIMEOUT_);
    if(status != HAL_OK)
    {
    ...
    status = HAL_I2C_Master_Receive(&hi2c1, OV7670_READ_ADDR_ << 1, &receivedData, 1, MAX_I2C_TIMEOUT_);
    if(status != HAL_OK)
    {
...
static void wrSensorRegs8_8(const regval_list reglist[]) {
...
void setColorSpace(enum COLORSPACE color) {
    switch (color) {
        ...
        case RGB565:
            ...
            {
                uint8_t temp = rdReg(0x11); // Assuming 0x11 is the correct register for your camera
                HAL_Delay(1); // 1 ms delay
                wrReg(0x11, temp); // Rewrite the PCLK register value as needed
            }
            break;
        case BAYER_RGB:
...
void setRes(enum RESOLUTION res) {
    switch (res) {
        case VGA:
            wrReg(REG_COM3, 0x00);  // Reset REG_COM3
            wrSensorRegs8_8(vga_ov7670);
            break;
        case QVGA:
            wrReg(REG_COM3, 0x04);  // REG_COM3 enable scaling
...
void camInit(void){
	...
	wrReg(REG_COM10, 0x20);//PCLK does not toggle on HBLANK.
}
